...
direccion_mem RAPCSMemoria::New_Espacio(string cad) {
	int x = 0;
...
void RAPCSMemoria::Delete_Espacio(direccion_mem dir) {
	int x = dir;
...
void RAPCSMemoria::Poner_dato(direccion_mem dir, string id, int valor) {
	if (id.substr(0, 2) == "->") //
		id.erase(0, 2);
	int z = dir;
	while (z != nulo) {
...
int RAPCSMemoria::obtener_dato(direccion_mem dir, string id) {
	if (id.substr(0, 2) == "->")
		id.erase(0, 2);
	int z = dir;
	while (z != nulo) {
...
int RAPCSMemoria::Espacio_Disponible() {
	int x = libre;
	int c = 0;
	while (x != nulo) {
		c++;
...
int RAPCSMemoria::Espacio_ocupado() {
	return max_memoria - Espacio_Disponible();
}

bool RAPCSMemoria::dir_libre(direccion_mem dir) {
	int x = libre;
	bool c = false;
...
void RAPCSMemoria::mostrar_memoria() {
	string esp7 = "";
	string linea = "";
	int esp = 10;
