...
direccion_mem CSMemoriaEOAO::New_Espacio(string cad) {
	int x = 0;
...
void CSMemoriaEOAO::Delete_Espacio(direccion_mem dir) {
	int x = dir;
...
void CSMemoriaEOAO::Poner_dato(direccion_mem dir, string id, int valor) {
	if (id.substr(0, 2) == "->") //
			id.erase(0, 2);
	int z = dir;
	while (z != nulo) {
...
int CSMemoriaEOAO::obtener_dato(direccion_mem dir, string id) {
	if (id.substr(0, 2) == "->")
		id.erase(0, 2);
	int z = dir;
	while (z != nulo) {
...
int CSMemoriaEOAO::Espacio_Disponible() {
	int x = libre;
	int c = 0;
	while (x != nulo) {
		c++;
...
int CSMemoriaEOAO::Espacio_ocupado() {
	return max_memoria - Espacio_Disponible();
}

bool CSMemoriaEOAO::dir_libre(direccion_mem dir) {
	int x = libre;
	bool c = false;
...
string rellenar(string s, int espacios) {
 ...
 void CSMemoriaEOAO::mostrar_memoria() {
 int esp = 10;
