...
direccion_mem GITCSMemoria::New_Espacio(string cad) {
	int x = 0;
...
void GITCSMemoria::Delete_Espacio(direccion_mem dir) {
	int x = dir;
...
void GITCSMemoria::Poner_dato(direccion_mem dir, string id, int valor) {
	if (id.substr(0, 2) == "->") //
		id.erase(0, 2);
	int z = dir;
	while (z != nulo) {
...
int GITCSMemoria::obtener_dato(direccion_mem dir, string id) {
	if (id.substr(0, 2) == "->")
		id.erase(0, 2);
	int z = dir;
	while (z != nulo) {
...
int GITCSMemoria::Espacio_Disponible() {
	int x = libre;
	int c = 0;
	while (x != nulo) {
		c++;
...
int GITCSMemoria::Espacio_ocupado() {
	return max_memoria - Espacio_Disponible();
}

bool GITCSMemoria::dir_libre(direccion_mem dir) {
	int x = libre;
	bool c = false;
...
void GITCSMemoria::mostrar_memoria() {
	string esp7 = "";
	string linea = "";
	int esp = 10;
