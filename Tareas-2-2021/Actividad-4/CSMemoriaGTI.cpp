...
direccion_mem CSMemoriaGTI::New_Espacio(string cad) {
	int x = 0;
...
void CSMemoriaGTI::Delete_Espacio(direccion_mem dir) {
	int x = dir;
...
void CSMemoriaGTI::Poner_dato(direccion_mem dir, string id, int valor) {
	if (id.substr(0, 2) == "->") //
		id.erase(0, 2);
	int z = dir;
	while (z != nulo) {
...
int CSMemoriaGTI::obtener_dato(direccion_mem dir, string id) {
	if (id.substr(0, 2) == "->")
		id.erase(0, 2);
	int z = dir;
	while (z != nulo) {
...
int CSMemoriaGTI::Espacio_Disponible() {
	int x = libre;
	int c = 0;
	while (x != nulo) {
		c++;
...
int CSMemoriaGTI::Espacio_ocupado() {
	return max_memoria - Espacio_Disponible();
}

bool CSMemoriaGTI::dir_libre(direccion_mem dir) {
	int x = libre;
	bool c = false;
...
void CSMemoriaGTI::mostrar_memoria() {
	int esp = 10;
	...
	string esp7 = "";
	string cabli = "LIBRES";
	string caboc ="OCUPADOS";
	string cabecera =" "+cabli+"                                   "+
		caboc+"\n"+"|" + cab_dir + "|" + cab_dato + "|" + cab_id +
		"|" + cab_link + "|" + cab_dir + "|" + cab_dato + "|" + cab_id +
		"|" + cab_link + "|";
	string linea = esp7 + repetir(cabecera.length() - esp7.length()-50, '-');
	cout << cabecera << endl;
	cout << linea << endl;
	for (int i = 0; i < max_memoria; i++) {
