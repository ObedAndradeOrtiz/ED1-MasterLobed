...
Direccion eoaoCSMemoria::New_Espacio(string cad) {
	int x = 0;
...
void eoaoCSMemoria::Delete_Espacio(Direccion dir) {
	int x = dir;
...
void eoaoCSMemoria::Poner_dato(Direccion dir, string id, int valor) {
	if (id.substr(0, 2) == "->") //
			id.erase(0, 2);
	int z = dir;
	while (z != nulo) {
...
int eoaoCSMemoria::obtener_dato(Direccion dir, string id) {
	if (id.substr(0, 2) == "->")
		id.erase(0, 2);
	int z = dir;
	while (z != nulo) {
...
int eoaoCSMemoria::Espacio_Disponible() {
	int x = libre;
	int c = 0;
	while (x != nulo) {
		c++;
...
int eoaoCSMemoria::Espacio_ocupado() {
	return MAXIMO - Espacio_Disponible();
}

bool eoaoCSMemoria::dir_libre(Direccion dir) {
	int x = libre;
	bool c = false;
...
string rellenar(string s, int espacios) {
 ...
 void eoaoCSMemoria::mostrar_memoria() {
 int esp = 10;
